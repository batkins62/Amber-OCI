...
public class Vault implements Hook
{
    ...
    public boolean chargeuser(double amount, Player player, String msg)
    {
        ...
        {
            Messager.sendMessage(player, Colors.Green, String.format("You were charged %s and now have %s", econ.format(r.amount), econ.format(r.balance)));
            Messager.info(player.getName() + " was charged $" + amount + " for " + msg);
            return true;
        }
        else
        {
        	Messager.sendMessage(player, Colors.Red, String.format("An error occured: %s", r.errorMessage));
        }
        return false;
    }
    ...
    public void payuser(Player player, double amount, String msg)
    {
        ...
        if(r.transactionSuccess())
        {
        	Messager.sendMessage(player, Colors.Green, String.format("You were given %s and now have %s for " + msg, econ.format(r.amount), econ.format(r.balance)));
        	Messager.info(player.getName() + " recieved $" + amount + " for " + msg);
        }
        else
        {
        	Messager.sendMessage(player, Colors.Red, String.format("An error occured: %s", r.errorMessage));
